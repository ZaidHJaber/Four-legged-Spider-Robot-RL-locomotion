...
% Desired height
h_final = 0.020 ; 
% Initial body height and spider displacement
init_6DOF_highet =h_final;
angles = InverseKinematics(init_6DOF_highet); %<<<
angles*180/pi
...
z_min = 0.01;                        % min height of body from ground
y_max = 0.05;                        % max translation in y
vx_max = 0.15;                         % max horizontal speed of body
vy_max = 0.025;                         % max vertical speed of body
vz_max = 0.025;                         % max lateral speed of body
...
%angles limits, 
min_tibiaRL_ang=-45 * d2r; % SV1,SV2 and SV3 joint angle limit
max_tibiaRL_ang=45 * d2r;

min_femurRL_ang= -45 * d2r; 
max_femurRL_ang = 49*d2r; 

min_coxaRL_ang = -30*d2r; 
max_coxaRL_ang = 60*d2r;

w_max = 2*pi*60/60;                 %joint angular speed limit
z_max = 0.001* (80*cos(min_tibiaRL_ang) - 55*sin(min_femurRL_ang)+20);  % max height of body from ground
